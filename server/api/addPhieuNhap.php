...
$required_fields = ["TrangThai", "IDTaiKhoan", "MaNhaCungCap", "NgayNhap"];
$missing_fields = [];
...
$idTaiKhoan = filter_var($data['IDTaiKhoan'], FILTER_VALIDATE_INT);
$maNhaCungCap = trim($data['MaNhaCungCap']);
$ngayNhap = trim($data['NgayNhap']);

// Kiểm tra IDTaiKhoan
if ($idTaiKhoan === false || $idTaiKhoan <= 0) {
    echo json_encode([
        "success" => false,
...
$sql = "INSERT INTO phieunhap (TrangThai, IDTaiKhoan, MaNhaCungCap, NgayNhap) VALUES (?, ?, ?, ?)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("siss", $trangThai, $idTaiKhoan, $maNhaCungCap, $ngayNhap);
...
if ($stmt->execute()) {
    ...
    echo json_encode(["success" => false, "message" => "Lỗi khi thêm phiếu nhập: " . $stmt->error]);
}
