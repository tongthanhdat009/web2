...
$required_fields = ["MaNhaCungCap", "TenNhaCungCap"];
$missing_fields = [];
...
if (!empty($missing_fields)) {
    $response = [
        "success" => false,
        "message" => "Vui lòng nhập đầy đủ thông tin nhà cung cấp",
...
$maNhaCungCap = trim($data['MaNhaCungCap']);
$tenNhaCungCap = trim($data['TenNhaCungCap']);

// Kiểm tra xem TenNhaCungCap đã tồn tại chưa
$checkNameSql = "SELECT TenNhaCungCap FROM nhacungcap WHERE TenNhaCungCap = ?";
$checkNameStmt = $conn->prepare($checkNameSql);
$checkNameStmt->bind_param("s", $tenNhaCungCap);
...
// Kiểm tra xem MaNhaCungCap đã tồn tại chưa
$checkSql = "SELECT MaNhaCungCap FROM nhacungcap WHERE MaNhaCungCap = ?";
$checkStmt = $conn->prepare($checkSql);
$checkStmt->bind_param("s", $maNhaCungCap);
$checkStmt->execute();
...
$stmt->bind_param("ss", $maNhaCungCap, $tenNhaCungCap);

// Thực hiện truy vấn và kiểm tra lỗi
if ($stmt->execute()) {
    echo json_encode(["success" => true, "message" => "Thêm nhà cung cấp thành công"]);
} else {
    echo json_encode(["success" => false, "message" => "Lỗi khi thêm nhà cung cấp: " . $stmt->error]);
}
